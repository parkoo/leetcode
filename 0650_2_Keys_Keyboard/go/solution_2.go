...
func minSteps(n int) int {
	...
	// 对于长度为i的字符串，若i是j的整数倍, 即（i%j==0, j<i）, 则长度为i的字符串可以由长度为j的字符经过1次拷贝以及（i/j-1）次粘贴操作等到,共（i/j）次操作， 即 dp[i] = dp[j] + (i/j)
	...
	// dp[12] =  dp[6] + 2  (对"AAAAAA"进行1次拷贝后再进行次粘贴)
	dp := make([]int, n+1)
